...
def downside_risk(data_bit,data_gold,money,alpha=0.8):
    ...
    for i in range(len(money)):
        add_value=data_mean*i+money[0]
        if money[i]<add_value:
            ans+=(money[i]-add_value)**2
            index+=1
...
def predict_value_with_exp_smoothing_3(data,params=[0.2,0.05,0.9],T=15,predict_step=15):
...
# 还有getresult所需要的全部数据
def get_result_by_state(state,df,real_last_day,predict_days,LSTM_data,
